...
std::string byte_array_t::as_string() const
{
  std::vector<char> tmp;
  tmp.reserve( size() * 2 );
...
inline std::size_t byte_array_t::word_size() const
{
  return align( size(), 2 ) / 2;
}

inline std::size_t byte_array_t::dword_size() const
{
...
byte_array_t& byte_array_t::append( const uint16_t val )
{
  m_vec.push_back( static_cast<char>( val >> 8 ) );
  m_vec.push_back( static_cast<char>( val & 0xFF ) );
  return *this;
}
...
byte_array_t& byte_array_t::append( const char* data, const std::size_t len )
{
  for ( std::size_t i = 0; i < len; ++i )
  {
...
byte_array_t& byte_array_t::append( const char* data, const std::size_t offset,
                                    const std::size_t len )
{
  for ( std::size_t i = 0; i < len; ++i )
  {
    m_vec.push_back( data[i + offset] );
  }
  return *this;
}

byte_array_t& byte_array_t::append( const byte_array_t& data,
